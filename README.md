...
这是一套面向复杂比稿、标书与经营提案的受控生产系统。它把资料审计、关键决策确认、策略与PPT提案成稿、视觉校准、HTML／Image2 页面生产、逐页批准、最终讲稿和 PPT 组装连接成一条可追溯工作流。
...
- 准确是边界，观点和决策推动是提案目标；策略目录前必须确认立场卡和全案情绪曲线。
- 正确但没有观点的页面退回；有观点但缺证据则补证；不能推动客户决策则重写。
- 总策略先编译为章节策略接口，再编译为页面论证链；上一章输出必须成为下一章输入。
- 目录、章节确认和内容冻结前执行只读标题测试；章节或页面可以任意换序时不得放行。
- 页面论证链确认后一次性生成整章正文并写入飞书，再逐页评论和确认；逐页确认不等于逐页串行生成。
- 飞书提案成稿按“核心内容＋逻辑展开＋上屏内容＋讲解方向＋策略过桥＋视觉生成建议＋来源”组织。
- 完整讲稿等待全部PPT页面、拆页和最终页序确认后，再按最终页码一次性生成。
...
- 基础交付：PNG、PDF、图片型 PPTX，优先保持批准视觉。
- 增强交付：按需增加带 PowerPoint 原生文字层的半可编辑 PPTX，并执行字段、坐标和渲染对照 QA。
- 不承诺图表、复杂 SVG、场景主视觉或 Image2 图像完全原生可编辑。HTML 与批准 PNG 仍是重建、复核和再次导出的权威来源。

...
形成有态度的总策略、客户目标决策、情绪曲线、章节策略接口和只读标题链。
...
- 全案情绪曲线。
- 动态目录与章节接口。
- 公司介绍与选定案例放在提案前部，沿用公司现有介绍逻辑；案例只证明与本项目相关的一项能力，不穿插进客户正式策略章节。
- 数据分析必须产出“数据事实 → 关系判断 → 品牌含义 → 策略选择 → 执行接口”，并把结论交给后续人群、货品、内容和经营章节。
- 目录确认后的项目 `AGENTS.md`。
- 模块模式只生成 `strategy_input → module_claim → strategy_output`、模块页面论证链和必要标题链；不得为了套完整流程虚构全案目录。

## 核心门禁

- 目录不能是标书模块重排。
- 上一章 `strategy_output` 必须等于下一章 `strategy_input`。
- 删除品牌名后仍可用于任意客户的判断不得通过。

## 禁止动作

- 目录未确认前不生成项目 `AGENTS.md`。
- 项目 `AGENTS.md` 未确认前不进入PPT成稿。

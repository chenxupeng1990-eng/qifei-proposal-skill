...
class SuiteDependencyTests(unittest.TestCase):
    def write_skill(self, root: Path, name: str) -> None:
        ...
        if name == "qifei-proposal-v2":
            manifest = __import__("json").loads((SKILL_ROOT / "suite.json").read_text(encoding="utf-8"))
            for relative in manifest["self_integrity"]["required_files"]:
    ...
    def test_incomplete_v2_installation_fails_even_when_entrypoint_exists(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            folder = root / "qifei-proposal-v2"
            folder.mkdir(parents=True)
            ...
            self.assertNotEqual(result.returncode, 0)
            self.assertIn("incomplete qifei-proposal-v2 installation", result.stdout)

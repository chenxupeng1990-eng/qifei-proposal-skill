...
16. 设计系统、飞书母稿和生成门禁全部通过后，页面生产可以按受控波次并行；主 Agent 持有共享权威文件，单个生产 Agent 只写自己分配的页面输出，硬上限为6个并行 Agent，默认3个。
17. 每个生产波次结束后，主 Agent 必须重新渲染、复核和记录结果，确认当前波次通过后才能启动下一波；并行生产不能改变策略、页序、设计语言或批准状态。
